...
os.environ["OPENAI_API_KEY"] = "sk-3SSseKA8p57h-UfQ9wBbmcxiyYycRPPWRhcg5ICY6ZT3BlbkFJEQIy8niz2IMuVp8QBrgSJAAxJa6hOXzHfKNqOgDLQA"  # Replace with your OpenAI API key
...
if not st.session_state.initialized:
    ...
    CAPACITIES = {truck: 1000 + 200 * (i % 5) for i, truck in enumerate(TRUCKS)}
...
sorting_option = st.selectbox("Sort by:", ['Show All', 'Truck', 'Departure Time', 'Depot Points'], index=0)
...
if sorting_option == 'Show All':
    ...
    show_all_fig, ax = st.session_state.network.draw_show_all_fig(best_solution=st.session_state.best_solution)
    st.pyplot(show_all_fig)

elif sorting_option == 'Truck':
    truck_option = st.selectbox("Choose Truck:", st.session_state.working_trucks, index=0)
    df = flattened_df[flattened_df['Truck'] == truck_option]
    st.table(df)
    truck_fig, ax = st.session_state.network.draw_truck_fig(truck_option, st.session_state.best_solution)
    if truck_fig:
        st.pyplot(truck_fig)

elif sorting_option == 'Departure Time':
    time_option = st.selectbox("Choose Departure Time:", st.session_state.working_times, index=0)
    df = flattened_df[flattened_df['Departure Time'] == time_option]
    st.table(df)
    time_fig, ax = st.session_state.network.draw_time_fig(time_option, st.session_state.best_solution)
    if time_fig:
        st.pyplot(time_fig)

elif sorting_option == 'Depot Points':
    depot_option = st.selectbox("Choose Depot:", st.session_state.depot_locations.keys(), index=0)
    depot_dict = {"TWTE": 0, "KSTP": 1, "TSIP": 2, "SWTE": 3}
    df = flattened_df[flattened_df['Locations'].apply(lambda x: x.split(' ')[0]) == str(st.session_state.network.depot_nodes[depot_dict[depot_option]])]
    st.table(df)
    depot_node = st.session_state.network.depot_nodes[depot_dict[depot_option]]
    depot_fig, ax = st.session_state.network.draw_depot_fig(depot_node, st.session_state.best_solution)
    if depot_fig:
        st.pyplot(depot_fig)
